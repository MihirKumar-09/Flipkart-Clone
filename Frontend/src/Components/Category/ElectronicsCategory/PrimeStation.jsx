...
export default function PrimeStation() {
  ...
  useEffect(() => {
    let fetchPorducts = async () => {
      try {
        let res = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/products/section?category=Console,Mouse,Keyboard,Headphones,Gaming Computer&limit=15&random=true`,
        );
