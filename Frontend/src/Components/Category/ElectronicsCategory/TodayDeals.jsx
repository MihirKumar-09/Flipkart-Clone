...
export default function AudioWave() {
  ...
  useEffect(() => {
    let fetchPorducts = async () => {
      try {
        let res = await fetch(
          `${process.env.VITE_BACKEND_URL}/api/products/section?category=Mouse,Keyboard,Men Trimmers,Women Trimmers,Nackbands,Earbuds,SoundSpeaker,Smart Watch,Console&limit=15&random=true`,
        );
