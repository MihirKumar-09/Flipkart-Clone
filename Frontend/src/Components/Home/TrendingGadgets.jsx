...
export default function LifeStyle() {
    ...
    const fetchTrendingGadgets = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/products/section?category=Nackbands,Earbuds,Men Trimmers,Women Trimmers&limit=4&random=true`,
        );
  ...
  useEffect(() => {
    const fetchBodyCare = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/products/section?category=Skin Care,Hair Care&limit=4&random=true`,
        );
        const data = await res.json();
        setBodyCare(data);
      } catch (err) {
  ...
  useEffect(() => {
    const fetchWomenFashion = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/products/section?category=Men's Shoes,Women's Shoes&limit=4&random=true`,
        );
        const data = await res.json();
