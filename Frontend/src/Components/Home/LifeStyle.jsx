...
export default function LifeStyle() {
    ...
    const fetchTopSection = async () => {
      try {
        const res = await fetch(
          "http://localhost:8080/api/products/section?category=Furniture,Laptops,Fitness&limit=4&random=true"
        );
  ...
  useEffect(() => {
    const fetchWomenFashion = async () => {
      try {
        const res = await fetch(
          "http://localhost:8080/api/products/section?category=Women's Fashion,BodyConeDress&limit=4&random=true"
        );
        const data = await res.json();
