...
export default function LifeStyle() {
    ...
    const fetchTopSection = async () => {
      try {
        const res = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/api/products/section?category=Furniture,Laptops,Fitness&limit=4&random=true`,
        );
  ...
  useEffect(() => {
    const fetchHomeProducts = async () => {
      try {
        const res = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/api/products/section?category=Home-Decoration&limit=4&random=true`,
        );
        const data = await res.json();
        setHomeStyle(data);
      } catch (err) {
  ...
  useEffect(() => {
    const fetchWomenFashion = async () => {
      try {
        const res = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/api/products/section?category=Women's Fashion,BodyConeDress&limit=4&random=true`,
        );
        const data = await res.json();
