...
export default function TrendingNow() {
    ...
    const fetchTopMobiles = async () => {
      try {
        const res = await fetch(
          "http://localhost:8080/api/products/section?category=Mobiles&limit=4&random=true"
        );
  ...
  useEffect(() => {
    const fetchTopRated = async () => {
      try {
        const res = await fetch(
          "http://localhost:8080/api/products/section?category=Laptops,Men's Fashion,Sports&limit=4&random=true"
        );
        const data = await res.json();
        setTopRated(data);
      } catch (err) {
