...
export default function Analytics() {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/admin/orders/analytics`,
      );
      setSummary(res.data);
    };
    fetchSummary();
  ...
  useEffect(() => {
    const fetchMonthlyRevenue = async () => {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/admin/orders/analytics/monthly-revenue`,
      );
  ...
  useEffect(() => {
    const fetchTopProducts = async () => {
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_BACKEND_URL}/api/admin/orders/analytics/top-products`,
  ...
  useEffect(() => {
    const fetchLowStockProducts = async () => {
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_BACKEND_URL}/api/admin/orders/analytics/low-stocks?threshold=5`,
          {
            withCredentials: true,
          },
        );
  ...
  useEffect(() => {
    const fetchDailyOrderStatus = async () => {
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_BACKEND_URL}/api/admin/orders/analytics/payment-method`,
          { withCredentials: true },
        );
